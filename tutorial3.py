...
class AssociativeBinaryBooleanOperation:
    ...
    @classmethod
    def random(cls, n_variables, q = None, j = None):
        """
        ...
        if q is None:
            q = 1 / np.log2(j + 1)
        n_literals = np.random.binomial(j, q)
    ...
    def evaluate(self, interpretations):
        ...
        """
        return self.fold((
                self.fold(~interpretations[:, self.negative_literals], axis=1),
                self.fold( interpretations[:, self.positive_literals], axis=1)),
                axis=0)
